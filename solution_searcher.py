...
class SolutionSearcher(Generic[T]):
    def __init__(
        ...
        counts: PieceKeyCounts,
        pieces: List[T],
        append_solution: Callable[[int, int], int],
        append_solution_with_prefix: Callable[[int, int, InsertNodeValue], int],
        trie_length_sum: int,
    ) -> None:
        if len(pieces) < 1:
            raise ValueError()
        ...
        self.search_stack: List[IterPieceKeyCountsPiece[T]] = [
            IterPieceKeyCountsPiece[T](piece) for piece in pieces
        ]
                ...
                if self.stack_index < self.last_stack_index:
                    if self.has_not_visited_before(piece):
                        ...
                        if piece.insert_node is not None:
                            insert_node_value = piece.insert_node.get_value(
                                piece.insert_index
                            )
                            if insert_node_value is not None:
                                solutions_count = self.append_solution_with_prefix(
                                    solutions_count, self.stack_index, insert_node_value
                                )
                            if print_visited_before_count:
                                print(TOP_LEFT)
                                print(
                                    f"{solutions_count}:{self.counts.root.leafs_count}/{self.counts.root.length}:{self.node_counter.count}/{self.trie_length_sum}"
                                )
                        else:
                            raise Exception()
                else:
                    solutions_count = self.append_solution(
                        solutions_count, self.stack_index
                    )
                    piece.last_next()
