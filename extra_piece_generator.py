...
class ExtraPieceKeyCountsPieceGenerator(PieceGenerator[ExtraPieceKeyCountsPiece]):
    def __init__(self, width: int, height: int, first_frame_piece_keys: List[str], piece_key_counts: Dict[str, Dict[str, List[PieceKeyCount]]], opposite_key: str = DEFAULT_OPPOSITE_KEY) -> None:
        ...
        def get_new_base_piece(frame_index: int, rotation_index: int, rotated: bool, directions: List[Directions], coordinate: Coordinate, edges: List[Edge]) -> ExtraPieceKeyCountsPiece:
            return ExtraPieceKeyCountsPiece(piece_key_counts, opposite_key, frame_index, rotation_index, rotated, directions, coordinate, edges)
        
        self.spiral: List[ExtraPieceKeyCountsPiece] = self.generate(get_new_base_piece)

        for i, piece_key in enumerate(first_frame_piece_keys):
            self.spiral[i].set_piece_key(piece_key)

        self.pieces: List[ExtraPieceKeyCountsPiece] = [piece for piece  in self.spiral if piece.frame_index > 0]
