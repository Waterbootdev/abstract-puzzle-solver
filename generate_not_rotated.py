...
def generate_not_rotated(
    rotated: List[bool], rotated_length: int, height: int, frame_indexes: List[int]
) -> List[List[int]]:
    ...
    if len(lengths) > 1:
        ...
        if len(lengths) > 2:
            between_first_and_last_frame(
                rotated, rotated_length, before, lengths, not_rotated
            )
...
def between_first_and_last_frame(
    rotated: List[bool],
    length: int,
    before: List[List[int]],
    lengths: List[int],
...
def last_frame(
    ...
    if height % 2 == 0:
        ...
        last_row(
            before, half_length - 2, last_frame_first_row(rotated, before, half_length)
        )
    else:
...
def first_frame(
    current_rotated: List[bool], before: List[List[int]], frame_length: int
):
    current_not_rotated: List[int] = []
